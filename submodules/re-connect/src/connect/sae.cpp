...
auto order_minus_1() -> crypto::Bn256 {
    auto r = crypto::Bn256();
    crypto::bn_sub(r, crypto::p256::order, crypto::Bn256::from_u32(1));
    return r;
}

// pwd-seed = HKDF-Extract(salt = ssid, ikm = password) = HMAC-SHA256(ssid, password)
auto pwd_seed(const noxx::Span<const u8> ssid, const noxx::Span<const u8> password, const crypto::Sha256::DigestMutRef out) -> void {
    const auto pieces = noxx::to_array<noxx::Span<const u8>>({password});
    crypto::hmac_sha256_vector(ssid, pieces, out);
}

// u = (HKDF-Expand(seed, info, 48) mod p), then SSWU(u)
auto sswu_from_seed(const crypto::Sha256::DigestRef seed, const char* const info) -> crypto::p256::Point {
    auto pwd_value = noxx::Array<u8, 48>(); // prime_len + ceil(prime_len/2)
    crypto::hkdf_expand(seed, span(info), pwd_value);
    return crypto::p256::sswu(crypto::bn_mod_bytes(pwd_value, crypto::p256::prime));
}
...
auto rand_below_order(crypto::Rng& rng, crypto::Bn256& out) -> bool {
    for(auto tries = 0; tries < 64; tries += 1) {
        auto buf = noxx::Array<u8, 32>();
        ensure(rng(buf));
        out = crypto::Bn256::from_be_bytes(buf.data);
        if(crypto::bn_cmp(out, crypto::p256::order) < 0) {
            return true;
        }
    }
...
// 1 < v < order
auto in_2_to_order(const crypto::Bn256& v) -> bool {
    return crypto::bn_cmp(v, crypto::Bn256::from_u32(1)) > 0 && crypto::bn_cmp(v, crypto::p256::order) < 0;
}

auto element_to_bin(const crypto::p256::Point& p, u8* const out) -> void {
    p.x.to_be_bytes(out);
...
auto derive_pt(const noxx::Span<const u8> ssid, const noxx::Span<const u8> password) -> Pt {
    auto seed = crypto::Sha256::Digest();
    ...
    return Pt{crypto::p256::add(p1, p2)};
}

auto Session::start(const Pt& pt, const MacAddrRef own_mac, const MacAddrRef peer_mac, crypto::Rng& rng) -> bool {
    ...
    auto       zero     = crypto::Sha256::Digest();
    const auto pieces   = noxx::to_array<noxx::Span<const u8>>({addrs});
    auto       val_hash = crypto::Sha256::Digest();
    crypto::hmac_sha256_vector(zero, pieces, val_hash);

    auto val = crypto::bn_mod_bytes(val_hash, order_minus_1());
    crypto::bn_add(val, val, crypto::Bn256::from_u32(1)); // 1 <= val <= q-1, no overflow
    pwe = crypto::p256::mul(pt.point, val);
    ensure(!pwe.inf && crypto::p256::is_on_curve(pwe), "degenerate PWE");

    // pick commit scalar/element: 1 < rand,mask < q with (rand+mask) mod q > 1
    for(auto tries = 0; tries < 64; tries += 1) {
        auto mask = crypto::Bn256();
        ensure(rand_below_order(rng, rand));
        ensure(rand_below_order(rng, mask));
        if(!in_2_to_order(rand) || !in_2_to_order(mask)) {
            continue;
        }
        own_scalar = crypto::p256::scalar_add(rand, mask);
        if(!in_2_to_order(own_scalar)) {
            continue;
        }
        own_element  = crypto::p256::negate(crypto::p256::mul(pwe, mask));
        committed    = true;
        have_keys    = false;
        send_confirm = 0;
        return true;
...
auto Session::read_commit(const noxx::Span<const u8> payload) -> bool {
    ...
    peer_scalar  = crypto::Bn256::from_be_bytes(payload.data + 2);
    peer_element = crypto::p256::Point{
        crypto::Bn256::from_be_bytes(payload.data + 2 + scalar_len),
        crypto::Bn256::from_be_bytes(payload.data + 2 + scalar_len + 32),
    };
    ...
    ensure(in_2_to_order(peer_scalar) || peer_scalar == crypto::Bn256::from_u32(1), "peer scalar out of range");
    ensure(!peer_scalar.is_zero() && crypto::bn_cmp(peer_scalar, crypto::p256::order) < 0);
    ensure(!peer_element.inf && crypto::p256::is_on_curve(peer_element), "peer element off curve");
    ...
    const auto k_point = crypto::p256::mul(crypto::p256::add(crypto::p256::mul(pwe, peer_scalar), peer_element), rand);
    ...
    auto       keyseed = crypto::Sha256::Digest();
    crypto::hmac_sha256_vector(zero, kpieces, keyseed);

    // context = (own_scalar + peer_scalar) mod order, 32-byte BE; PMKID = first 16
    const auto sum     = crypto::p256::scalar_add(own_scalar, peer_scalar);
    ...
    crypto::sha256_prf(keyseed, span("SAE KCK and PMK"), context, out);
...
auto confirm_hash(const KCKRef kck, const u8* const sc,
                  const crypto::Bn256& scalar1, const crypto::p256::Point& element1,
                  const crypto::Bn256& scalar2, const crypto::p256::Point& element2, crypto::Sha256::DigestMutRef out) -> void {
    ...
    crypto::hmac_sha256_vector(kck, pieces, out);
}
} // namespace
...
auto Session::write_confirm(noxx::Span<u8> out) -> usize {
    ...
    ensure(out.size() >= 2 + crypto::Sha256::Digest::size());
    send_confirm += 1;
    ...
    return 2 + crypto::Sha256::Digest::size();
}

auto Session::verify_confirm(const noxx::Span<const u8> payload) const -> bool {
    ensure(have_keys);
    ensure(payload.size() >= 2 + crypto::Sha256::Digest::size(), "short confirm");
    auto verifier = crypto::Sha256::Digest();
